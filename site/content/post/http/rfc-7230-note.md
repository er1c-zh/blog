...
`Via`首部字段存储了用户代理到服务器之间的中间件协议 *（intermediate protocols）* 和收件人 *（recipients）* 。
...
这些信息可以用来推断出哪些特性可以被安全的使用于响应或后续的请求中。
...
客户端应该限制与同一个服务端的同时开启的链接的数量。

本版本的协议不再给出一个绝对的数量限制，
而是鼓励客户端在开启更多的链接时，保持谨慎。
...
服务端接受到`close`时，
MUST在发送完成对应于带有`close`的请求的响应报文时，
开始将这个链接关闭。
...
1. 服务端等待并读取报文，
直到接受到客户端发来确认关闭的报文或服务端能够确认用户端已经消费到了最后的响应。
1. 关闭链接。
