...
请求中的Host首部字段提供了目标URI的host和端口信息，
这为在一个ip上部署的服务端程序提供了区分多个主机名称的能力。

客户端MUST发送Host首部字段。

代理接受到一个`absolute-form`格式的请求时，
MUST忽略请求中的Host首部字段，
并由`request-target`中的主机信息替代。

由于Host首部字段被当作应用级别的路由机制，
经常会有针对Host首部字段的攻击，
如污染缓存、重定向到非预期的服务器。

服务端MUST针对缺少Host首部字段、多个首部字段或非法Host字段值的请求返回400错误。
...
由于HTTP请求没有特定的字段等标识符将请求于响应关联起来，
所以对于同一个链接上的请求，
完全依赖发送请求的顺序和响应发送回客户端的顺序来匹配请求和响应。
...
除隧道类型的中间件，
其他中间件MUST实现`Connection`的首部字段，
并排除只用于incoming connection的字段。

中间件MUST NOT转发发送给自己的报文，
除非不会陷入无限循环的情况。

### 5.7.1 Via字段

`Via`首部字段存储了用户代理到服务器之间的中间件协议 *（intermediate protocols）* 和收件人 *（recipients）* 。

`Via`字段用来主从转发避免循环，
和识别在请求响应链中的发送者的协议能力 *（protocol capabilities）* 。

`Via`可能存在多个值，
分别代表了整个链条中转发过这个请求的代理或网关。
每个中间件都会追加“如何收到此报文”的信息到其中。

`Via`中的一项由接受的协议 *（received-protocol）*、
标识当前接受者信息的 *（received-by）* 以及可选的备注构成。
**received-by**由`uri-host:port/token`构成。

代理MUST生成并发送适当的`Via`字段。
HTTP到HTTP网关MUST为流向服务器的请求报文生成并发送适当的`Via`字段，
MAY为响应生成并发送`Via`。

对于每个中间件来说，`received-protocol`表明了上游发送者使用的协议以及版本。
这些信息可以用来推断出哪些特性可以被安全的使用于响应或后续的请求中。

`received-by`部分通常是报文传输链条中的接受者的IP与PORT。

发送者MAY为自己的`Via`记录生成备注，
可以用来区分接受者的软件类型，
作用类似于`User-Agent`和`Server`字段。

对于连接防火墙内外的中间件，
SHOULD NOT将防火墙内的转发节点写入到`Via`字段中的`received-by`信息直接转发到外部，
作为替代，需要将`received-by`信息映射为代称 *（pseudonym）* 。

### 5.7.2 Transformations

某些种类的中间件会在转发时将报文的载荷或内容做一些转换，
比如，可能尝试转换图片的编码格式来减少缓存时占用的空间
或减少与上下游传输的数据量。
...
HTTP报文传输与下层的传输层链接协议或会话层链接协议互相独立。
HTTP假定下层的链接是能够按照顺序投递请求和接受响应完成可靠的传输的链接。

根据5.2节，
使用哪种链接协议由客户端的配置和目标URI决定的。

实现HTTP的组件被期望参与到链接的管理工作中，
包括维护当前链接的状态、
建立新的链接或重用已存在的链接、
处理链接上接受到的报文、
检测链接错误、
...
发送者使用`Connection`首部字段表明对于该链接的控制选项。
出于避免下游接受者错误的理解`Connection`的原因，
PROXY和GATEWAY在转发报文前MUST移除或者替换从上游接受到的报文的控制选项。

发送者MUST将用于提供控制信息的首部字段列入`Connection`首部字段。
类似于`Connection`，
PROXY和GATEWAY在转发报文时，
需要解析出这些提供控制信息的首部字段，
并移除或替换掉。

这种机制为链接管理提供了声明式的控制逐跳的链接的能力，
又因为有自明性，可以较容易的加入新的控制选项。

`Connection`字段值中，
除了首部字段名，
还有可能是`close`。
`close`表明发送者在完成这一次请求响应之后，
会关闭这个链接。

不支持持久链接的客户端MUST要发送`close`。

不支持持久链接的服务端除了响应报文是1XX类型时，MUST发送`close`。
...
HTTP/1.1及之后的版本的实现SHOULD支持持久化链接。

接受者判断该链接是否是持久化链接根据如下的原则：

1. 如果设置了`close`选项，那么这个链接不是持久化链接。
1. 如果版本是HTTP/1.1或之后的版本，那么认为是持久化的。
1. 如果版本是1.0，且设置了`keep-alive`选项，且接受者不是proxy，且支持`keep-alive`机制，那么认为是持久化的。
1. 否则，认为不是持久化的。

客户端MAY向持久化链接追加请求，直到发送或接受到`close`的报文或没有`keep-alive`的1.0版本的报文。

为了实现链接的持久化，所有传输的报文一定要有定义好的长度，
而不是依赖链接的关闭作为报文结束的标志。
服务端MUST读取完每个请求或在完成发送该响应之后就关闭链接，
这可以避免因为残余的请求报文影响下个报文的解析。
相似的，客户端也要完成这样的工作。

代理MUST NOT与1.0版本的客户端维持持久化链接。

### 6.3.1 重试请求

设定上，HTTP请求的链接可能在任何时候计划或意外的中断。
HTTP的实现需要兼容这种情况，
拥有从意外关闭中恢复的能力。
...
用户代理MUST NOT自动重试 **非幂等方法的请求**。

客户端SHOULD NOT自动重试失败的自动重试。

### 6.3.2 流水线 pipelining

支持持久链接的客户端MAY流水线化多个请求。
如果这些请求是 **安全的方法** 的请求，
服务端MAY并行处理这些请求，
返回结果的时候，
服务端还是需要按照请求发送的顺序来返回响应。

关于安全的方法更多的在RFC7231 4.2.1节。

如果在接受到所有的请求之前链接被关闭，
流水线化请求的客户端SHOULD重试服务端没有响应的请求。
对于这种情况触发的用于重试的链接，
客户端MUST NOT立即流水线化第一个没有接受到结果的请求，
因为这个请求可能是导致上次链接中断的原因。

在一个流水线中，
用户代理SHOULD NOT在发送了一个非幂等方法的请求之后，
再次追加请求，直到响应的状态码返回。
除非用户代理能够从部分失败的情况中恢复。

中间件MAY流水线化接受到的流水线化的inbound请求，
...
客户端应该限制与同一个服务端的同时开启的链接的数量。

本版本的协议不再给出一个绝对的数量限制，
而是鼓励客户端在开启更多的链接时，保持谨慎。

开启多个链接的一个典型场景是为了解决在单一链接上流水线化的请求
可能会被一个处理或传输比较耗时的请求阻塞的问题。
考虑到开启多个链接会消耗服务端与网络的资源，
可能会在拥塞的网络条件下表现更差。

服务端有可能会出于自身保护的目的来拒绝一些请求。
...
当链接的任何一端希望关闭链接时，
SHOULD在报文的Connection字段中写入`close`。

客户端写入了`close`之后，
MUST NOT再向同一个链接追加写入请求；
MUST在接受到对应最后一个请求的响应报文时关闭链接。

服务端接受到`close`时，
MUST在发送完成对应于带有`close`的请求的响应报文时，
开始将这个链接关闭。
服务端SHOULD在最后的报文中携带`close`。
服务端MUST NOT在这之后处理这个链接上的任何请求。

服务端发送完（这里区别于接受是被动的）`close`后，
MUST在发送完成后，开始将这个链接关闭。
服务端MUST NOT在这之后处理这个链接上的任何请求。

客户端接受到`close`之后，
MUST取消正在这个链接上发送的请求，
并在读取完带有`close`的响应报文之后，
关闭这个链接。
对于正在流水线中的请求，
客户端SHOULD NOT假定它们会被处理。

如果服务端忽视时机立即关闭一个TCP链接，
有导致客户端无法读取到最后的响应的风险。
考虑一个场景，
客户端在接受到`close`响应之前，继续发送了一个报文，
服务端此时已经关闭了这个TCP链接，
会导致服务端向客户端发送一个reset报文，
这会导致客户端未被应用层消费的HTTP报文被丢弃。

为了避免这个问题，
服务端关闭链接时需要按照一定的步骤。
1. 服务端首先进入半关闭状态，只关闭写入。
1. 服务端等待并读取报文，
直到接受到客户端发来确认关闭的报文或服务端能够确认用户端已经消费到了最后的响应。
1. 关闭链接。

## 6.7 升级

`Upgrade`字段提供了一种简单机制用于实现在一个链接上将应用层协议从HTTP/1.1切换到其他协议。
客户端MAY在`Upgrade`字段中传递希望切换到的协议列表，
有先后顺序。
服务端MAY忽略`Upgrade`字段，
继续用当前协议进行传输。

服务端在发送101（Switch Protocols）请求时，
MUST发送`Upgrade`字段指出将要切换到的协议，
如果有多层协议要切换，发送者MUST将协议按序排列。
服务端MUST NOT切换到客户端没有在当前请求在`Upgrade`字段中指出的协议。
服务端MAY忽略请求中的`Upgrade`顺序，
转而因为其他因素来选择要切换到的协议。

服务端发送一个426（Upgrade Required）响应时，
MUST发送`Upgrade`字段来指明支持的协议，
顺序反应服务端的偏好。

服务端MAY在任何请求的响应中设置`Upgrade`字段，
用于表明未来的请求可以使用这些协议。

服务端决定切换协议之后，
发送了一个101响应，然后可以直接发送新协议下对于原请求的响应。

当发送一个`Upgrade`报文时，
MUST将`Upgrade`字段包含到`Connection`字段中，
这可以避免中间件将自己不支持的协议类型转发。

客户端在“正在”传输一个请求时不能进行协议升级。
服务端在接受到了一个同时设置了`Upgrade`和`Expect`首部字段的请求时，
MUST先发送包含100（Continue）的响应，
然后在发送101（Switching Protocols）的响应。

`Upgrade`首部字段只能用来切换该链接的顶层的协议，
这意味着无法切换传输层协议或切换传输使用的链接。
为了解决上述的场景，
可以使用3xx（Redirection）报文来处理。

# 7 ABNF List Extension: #rule

`#rule`在定义语法时用来当作“注释”性质，
提高定义的可读性。
...
关于HTTP报文的语法、解析和路由的安全考虑。
关于语义和载荷的安全考虑在RFC7231。
