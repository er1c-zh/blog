...
为了避免攻击者通过伪造websocket的帧来攻击如代理等的互联网基础设施，
协议增加了mask机制，
可以参考[这里](https://tools.ietf.org/html/rfc6455#section-10.3)。

mask机制要求客户端发送给服务器的请求的数据，
使用每次都随机且以难以预测的方式生成的Masking key进行按特定算法进行转换。
服务器在接受到这些数据后会相应的转换，获得数据。
...
- 0x8 关闭链接
  - 用来关闭websocket链接，载荷是关闭链接的原因
- 0x9 ping
  - 当收到ping帧时，除了如关闭等特殊情况，一定要尽快向另一端返回pong帧
- 0xA pong
  - 用于回应ping帧
...
将接收到的数据按照协议中的帧的格式进行解析：
1. 如果是控制帧则执行相应的操作
1. 如果是数据帧，则继续接受直到最后一帧到达。
